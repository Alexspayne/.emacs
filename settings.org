...
** Claude AI (gptel)
Integration with Claude via the [[https://github.com/karthink/gptel][gptel]] package, which supports Anthropic natively
along with streaming responses and org-mode awareness.

Requires an Anthropic API key. Set it in your shell environment before starting Emacs:
#+begin_example
export ANTHROPIC_API_KEY="sk-ant-..."
#+end_example

Keybindings:
- ~C-c l~ — open a Claude chat buffer
- ~C-c L~ — send the current prompt (or selected region from any buffer)
- ~C-c M-l~ — rewrite/refactor selected region in-place

Note: ~C-c C-l~ is intentionally avoided (org-insert-link) and ~C-c RET~ is
avoided inside org tables (org-table-hline-and-move).

#+BEGIN_SRC emacs-lisp :results output silent
(use-package gptel
  :ensure t
  :config
  ;; Set Anthropic/Claude as the default backend
  (setq gptel-backend
  ...
  ;; claude-sonnet is a good daily-driver; swap for claude-opus-4-6 for harder tasks
  (setq gptel-model 'claude-sonnet-4-6)
  ;; Integrate with org-mode so responses use org markup
  (setq gptel-default-mode 'org-mode)
  :bind
  (("C-c l"   . gptel)
   ("C-c L"   . gptel-send)
   ("C-c M-l" . gptel-rewrite)))
#+END_SRC
